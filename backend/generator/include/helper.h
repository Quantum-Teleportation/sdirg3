...
#define DEFINE_CLASSNAME(name) std::string_view GetName() const override { return #name; }
...
#define REGISTER_CLASS(base, name)                                                   \
	namespace {                                                                \
	bool ___XXX_##name##_XXX___ = HelperFactory<base>::GetInstance().RegisterClass(  \
		#name, defaultCreator<base, name>);                                          \
	}

#define CREATE_CLASS(base, name)                                                     \
    static_cast<name *>(HelperFactory<base>::GetInstance().CreateClassByName(#name))
